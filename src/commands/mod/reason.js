...
const { util, Command } = require("klasa")
const { EMOTES: {cross, check, no}} = require("../../lib/util/constants")
...
module.exports = class extends Command {
    ...
    async run(msg, [selected, ...reason]) {
        ...
        if (!log) return msg.sendMessage(`${cross} ${msg.author}, That case could not be found, please try another ID.`);

        if (!msg.guild.settings.get("channels.modlogs")) return msg.sendMessage(`${cross} Modlogs channel not found. Please do \`${msg.guild.settings.get("prefix")}modlogschannel <channel>\` to set it.`);
        const channel = msg.guild.channels.cache.get(msg.guild.settings.get("channels.modlogs"));
        if (!channel) return msg.sendMessage(`${cross} Modlogs channel not found. Please do \`${msg.guild.settings.get("prefix")}modlogschannel <channel>\` to set it.`);
        ...
        await msg.guild.settings.update("modlogs", logs[selected].reason);

        return msg.send(`${check} Case ${selected} has been updated.${util.codeBlock("http", [
